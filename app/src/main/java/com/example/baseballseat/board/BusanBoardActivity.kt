...
class BusanBoardActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.busanBoardRv.adapter = adapter
        binding.busanBoardRv.layoutManager = LinearLayoutManager(this)

        update_RecyclerView()

        binding.busanBoardRv.addOnScrollListener(object : RecyclerView.OnScrollListener(){
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                super.onScrolled(recyclerView, dx, dy)
                val lastItemPosition =
                    (binding.busanBoardRv.layoutManager as LinearLayoutManager).findLastCompletelyVisibleItemPosition()

                val itemTotalCount = binding.busanBoardRv.adapter?.itemCount?.minus(1)
